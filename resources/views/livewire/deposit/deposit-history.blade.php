<div>
    <section class="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
            ...
            <h1 class="mt-1.5 text-2xl font-semibold tracking-tight text-slate-900 dark:text-white sm:text-3xl">Deposit history</h1>
            <p class="mt-1.5 text-sm text-slate-500 dark:text-slate-400">Every deposit made to your account.</p>
        </div>
    </section>
        ...
        @forelse($this->deposits as $deposit)
            @php
                ...
                $statusStyle = match($status) {
                    \App\Enums\DepositStatus::Confirmed => ['bg-emerald-500/12 text-emerald-500', 'bg-emerald-500/12 text-emerald-600 dark:text-emerald-400'],
                    \App\Enums\DepositStatus::Pending => ['bg-amber-500/12 text-amber-500', 'bg-amber-500/12 text-amber-600 dark:text-amber-400'],
                    \App\Enums\DepositStatus::Rejected => ['bg-rose-500/12 text-rose-500', 'bg-rose-500/12 text-rose-600 dark:text-rose-400'],
                };
            @endphp

            <button wire:click="view({{ $deposit->id }})" wire:key="deposit-{{ $deposit->id }}" class="flex w-full items-center gap-3 border-b p-4 sm:p-5 text-left transition last:border-b-0 border-slate-100 hover:bg-slate-50 dark:border-white/[.06] dark:hover:bg-white/[.03]">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2">
                        <b class="truncate text-sm text-slate-900 dark:text-white">{{ $deposit->method }} deposit</b>
                    </div>
                    <small class="text-xs text-slate-500 dark:text-slate-400">{{ $deposit->transaction_id }} · {{ time_ago($deposit->created_at) }}</small>
                </div>

                <div class="shrink-0 text-right">
                    <b class="block text-sm text-emerald-600 dark:text-emerald-500">
                        +{{ money_format($deposit->amount, $deposit->currency) }}
                    </b>
    ...
    @if($this->deposits->hasPages())
        <div class="mt-4">
            {{ $this->deposits->links('vendor.pagination.custom') }}
        </div>
    @endif
        ...
        >
            @if($this->selectedDeposit)
                ...
                <div class="sticky top-0 z-10 flex items-center justify-between border-b p-5 border-slate-200 bg-white/90 backdrop-blur-xl dark:border-white/[.08] dark:bg-[#0f172a]/90">
                    <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Deposit details</h2>
                ...
                <div class="p-5">
                    ...
                    <div class="flex flex-col items-center text-center">
                        ...
                        <p class="mt-3 text-2xl font-semibold tracking-tight text-emerald-600 dark:text-emerald-500">
                            +{{ smart_money($d->amount, $d->currency) }}
                        </p>
                    ...
                    @if($status === \App\Enums\DepositStatus::Rejected && $d->rejection_reason)
                    ...
                    {{-- Pending notice --}}
                    @if($status === \App\Enums\DepositStatus::Pending)
                        <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3.5 text-sm text-amber-700 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-400">
                            This deposit is awaiting confirmation. You'll be notified once it's approved.
                        </div>
                    @endif
                    ...
                    <div class="mt-6 space-y-1 divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-white/[.06] dark:border-white/[.08]">
                        <div class="flex items-center justify-between p-3.5">
                            ...
                            <div class="flex items-center gap-1.5">
                                <b class="text-sm text-slate-900 dark:text-white">{{ $d->transaction_id }}</b>
                                <button
                                    x-data="{ copied: false }"
                                    @click="navigator.clipboard.writeText('{{ $d->transaction_id }}'); copied = true; setTimeout(() => copied = false, 1500)"
                        ...
                        <div class="flex items-center justify-between p-3.5">
                            ...
                            <b class="text-sm text-slate-900 dark:text-white">{{ $d->method }}</b>
                        </div>

                        <div class="flex items-center justify-between p-3.5">
                            <span class="text-xs text-slate-500 dark:text-slate-400">Amount</span>
                            <b class="text-sm text-slate-900 dark:text-white">{{ smart_money($d->amount, $d->currency) }}</b>
                        </div>

                        @if($d->fee > 0)
                            <div class="flex items-center justify-between p-3.5">
                                <span class="text-xs text-slate-500 dark:text-slate-400">Fee</span>
                                <b class="text-sm text-slate-900 dark:text-white">{{ smart_money($d->fee, $d->currency) }}</b>
                            </div>
                        @endif

                        <div class="flex items-center justify-between p-3.5">
                            <span class="text-xs text-slate-500 dark:text-slate-400">Currency</span>
                            <b class="text-sm text-slate-900 dark:text-white">{{ $d->currency }}</b>
                        </div>

                        <div class="flex items-center justify-between p-3.5">
                            <span class="text-xs text-slate-500 dark:text-slate-400">Date</span>
                            <b class="text-sm text-slate-900 dark:text-white">{{ $d->created_at->format('M j, Y · g:i A') }}</b>
                        </div>
                    </div>
                </div>
            @endif
